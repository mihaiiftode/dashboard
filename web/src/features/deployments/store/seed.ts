...
import { createFetchDeploymentsApi, PULL_BATCH_SIZE } from "./api"
...
export const seedRows = async (maximum: number): Promise<Deployment[]> => {
  ...
  while (rows.length < maximum) {
    ...
    const page: DeploymentPage = await api.list({ after, limit: Math.min(PULL_BATCH_SIZE, maximum - rows.length) })
