...
import { openDeploymentsPage, renderDeploymentsPage, rowNamed, type PageOptions } from "@/test/deployments-page"
import { setupUser } from "@/test/user"

const TWO_ROUND_TRIPS = 15_000

const open = async (options: PageOptions = {}) => {
  const user = setupUser()
  return { user, ...(await openDeploymentsPage({ rows: deployments(6), footer: true, ...options })) }
}

const openEmpty = async (query: string, rows: Deployment[]) => {
  const rendered = renderDeploymentsPage({ query, footer: true, rows })
...
describe("deleting and restoring", () => {
  ...
  it("lists only deleted deployments under the deleted scope, read only, with the days left", async () => {
    ...
    const { table } = await open({ query: "is:deleted", rows })
  ...
  it("returns a restored deployment to the default scope", async () => {
    const rows = [deployment(0), deployment(1, { deleted_at: deletedDaysAgo(2) })]
    const { user, table, api } = await open({ query: "is:deleted", rows })
